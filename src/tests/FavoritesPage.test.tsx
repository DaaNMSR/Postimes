...
const getElements = () => {
  const linkElement = screen.queryByRole('link', { name: 'To Github search...' }) as HTMLLinkElement;
  const paragraphEmptyFavorites = screen.queryByText('No favourites.') as HTMLParagraphElement;
  const paragraphHasFavorites = screen.queryByText('My favorites') as HTMLParagraphElement;
...
describe('FavoritesPage component', () => {
  ...
  it('display items when we have favorites, and does not display link and - No favorites', () => {
    const favorites = ['https://github.com/DaaNMSR/SINDFIS.fy', 'https://github.com/DaaNMSR/aksimym.ru'];
    renderWithStore({ github: { favorites } });

    const { linkElement,paragraphEmptyFavorites,paragraphHasFavorites,listItems } = getElements();
